...
class PartialUpdateStationsAndPricesJob(
  ...
  def receive: Receive = {
    case RunJob => {
      if (appConfig.jobPartialIsEnabled) {
        ...
        db.withTransaction { implicit conn =>
          ...
          lastUpdate match {
            ...
            case Some(lastUpdate) =>
              val execute = for {
                _ <- fuelPriceService.uploadAllFuelStations(effectiveStartDate = Some(lastUpdate)).leftMap(error => s"Error while retrieving stations: ${error.message}")
                _ <- fuelPriceService.uploadAllFuelPrices(effectiveStartDate = Some(lastUpdate)).leftMap(error => s"Error while retrieving fuel prices: ${error.message}")
              } yield ()
        ...
        logger.info("Partial update is disabled, skipping job")
      }
    }
  }
}
