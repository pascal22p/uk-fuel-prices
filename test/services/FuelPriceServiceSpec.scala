...
class FuelPriceServiceSpec extends BaseSpec {
    ...
    "upload data recursively until not found" in {
      when(mockFuelPriceConnector.fuelStations(any(), any())(using any())).thenReturn(
        EitherT.rightT[Future, UpstreamErrorResponse](Seq(
          FuelStation("nodeId1", "tradingName", None, "brandName", None, None, None, None, FuelStationLocation(None, None, "city", None, None, "postcode", None), List.empty)
        )),
        EitherT.rightT[Future, UpstreamErrorResponse](Seq(
          FuelStation("nodeId2", "tradingName", None, "brandName", None, None, None, None, FuelStationLocation(None, None, "city", None, None, "postcode", None), List.empty)
        )),
  ...
  "uploadAllFuelPrices" must {
    ...
    "Ignore invalid node ids" in {
      ...
      argumentCaptorFuelPrices.getAllValues.asScala.toSeq.flatten.flatMap(_.fuelPrices).map(_.price) mustBe Seq(148.0, 153.0)
    ...
    "Stop recursion if the sanitised list is empty" in {
      ...
      result mustBe Right(true)
      verify(mockFuelPriceConnector, times(2)).fuelPrices(any(), any())(using any())
      verify(mockInsertSqlQueries, times(1)).insertFuelPrices(argumentCaptorFuelPrices.capture())
