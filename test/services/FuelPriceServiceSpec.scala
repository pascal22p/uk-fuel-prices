    ...
    "upload data recursively until not found" in {
      when(mockFuelPriceConnector.fuelStations(any(), any())(using any())).thenReturn(
        EitherT.rightT[Future, UpstreamErrorResponse](Seq(
          FuelStation("nodeId1", "tradingName", None, "brandName", None, None, None, None, fakeFuelStationLocation(), List.empty)
        )),
        EitherT.rightT[Future, UpstreamErrorResponse](Seq(
          FuelStation("nodeId2", "tradingName", None, "brandName", None, None, None, None, fakeFuelStationLocation(), List.empty)
        )),
